...
$token = $userDb->prepare("SELECT token FROM users WHERE username = :username");
$token->bindValue(':username', $username, SQLITE3_TEXT);
$r = $token->execute()->fetchArray(SQLITE3_ASSOC);

if (!$r || $r["token"] !== $sessionToken) {
